...
export class AuthController {
  ...
  @ApiResponse({
    ...
    schema: { example: { access_token: 'jwt.token.here', expires_in: 3600 } },
  })
  @Post('login')
  ...
  @ApiBody({
    schema: {
      properties: {
        clientId: { type: 'string', example: 'clientid123' },
        clientSecret: { type: 'string', example: 'secretkey' },
      },
    ...
    examples: {
      client: {
        ...
        value: { clientId: 'clientid123', clientSecret: 'secretkey' },
      },
    },
  })
  @ApiResponse({
    status: 200,
    description: 'JWT access token for client',
    schema: { example: { access_token: 'jwt.token.here', expires_in: 3600 } },
  })
  @Post('token')
...
export class OAuthController {
  ...
  @ApiResponse({
    ...
    schema: { example: { code: 'authcode123', state: 'xyz' } },
  })
  @Get('authorize')
  async authorize(
    ...
    if (responseType !== 'code')
      throw new HttpException('Only code supported', HttpStatus.BAD_REQUEST);
    ...
    await this.codeRepo.save(
      this.codeRepo.create({ code, client, user, redirectUri, expiresAt }),
    );
  ...
  async token(@Body() body: any) {
    if (body.grant_type === 'authorization_code') {
      ...
      if (
        codeEntity.client.clientId !== body.client_id ||
        codeEntity.client.clientSecret !== body.client_secret
      ) {
        throw new HttpException(
          'Invalid client credentials',
          HttpStatus.UNAUTHORIZED,
      ...
      await this.refreshRepo.save(
        this.refreshRepo.create({
          token: refreshToken,
          client: codeEntity.client,
          user: codeEntity.user,
      ...
      if (
        refresh.client.clientId !== body.client_id ||
        refresh.client.clientSecret !== body.client_secret
      ) {
        throw new HttpException(
          'Invalid client credentials',
          HttpStatus.UNAUTHORIZED,
